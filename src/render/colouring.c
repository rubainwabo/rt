...
void		shading_far(t_rt *specs, t_ray ray, int x, int y)
{
	...
	specs->img_str[(y * W_IMG + x) * 4] = (int)(skybox.z * 255.0) & 0xff;
	specs->img_str[(y * W_IMG + x) * 4 + 1] = (int)(skybox.y * 255.0) & 0xff;
	specs->img_str[(y * W_IMG + x) * 4 + 2] = (int)(skybox.x * 255.0) & 0xff;
}
