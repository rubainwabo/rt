...
void		apply_sepia(t_rt *specs)
{
	...
	while (i < comp)
	{
		...
		v.green = ft_min(255.0, (((unsigned char)specs->img_s->data[i + 2] * .349) +
		((unsigned char)specs->img_s->data[i + 1] * .686) +
		((unsigned char)specs->img_s->data[i] * .168)));
		v.blue = ft_min(255.0, (((unsigned char)specs->img_s->data[i + 2] * .272) +
		((unsigned char)specs->img_s->data[i + 1] * .534) +
